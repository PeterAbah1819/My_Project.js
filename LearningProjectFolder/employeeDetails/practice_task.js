const employees = [
    ...
    { id: 4, name: 'Mary Jane', age: 32, department: 'IT', salary: 55000, specialization: 'JavaScript' }
  ];
  ...
  function displayEmployees() {
    const totalEmployees = employees.map(employee => `
      <p>
        ${employee.id}: ${employee.name} - ${employee.department} - $${employee.salary} - ${employee.specialization}
      </p>`).join('');
  ...
  function calculateTotalSalaries() {
    ...
    document.getElementById('totalSalariesDisplay').innerHTML = `Total Salaries: $${totalSalaries}`;
  }
  ...
  // Find employees by specialization
  function findEmployeesBySpecialization(specialization) {
    ...
    if (specializedEmployees.length > 0) {
      const display = specializedEmployees.map(employee => `
        <p>
          ${employee.id}: ${employee.name} - ${employee.department} - $${employee.salary} - ${employee.specialization}
        </p>`).join('');
